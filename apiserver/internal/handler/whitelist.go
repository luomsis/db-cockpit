...
// resolveMetaInstance UI 演示实例 → 元数据域 db_instance。三级匹配：
// ① 名称精确匹配；② endpoint=ip:port；③ OB 约定兜底——observer 主机按租户
// extensions.units（unit 所在 server_ip）匹配租户。均跳过 sys 租户（端点歧义）。
// 解析失败返回 nil（调用方回退全局）。
func (h *H) resolveMetaInstance(iid string) *int64 {
	...
	err := h.DB.Where("role <> 'sys' AND (name = ? OR endpoint = ?)", inst.Name,
		fmt.Sprintf("%s:%d", inst.IP, inst.Port)).Order("id asc").First(&m).Error
	if err != nil {
		units := fmt.Sprintf(`[{"server_ip":%q}]`, inst.IP)
		err = h.DB.Where("role <> 'sys' AND extensions->'units' @> ?::jsonb", units).
...
// dbTypes 元数据域活计数；db_cluster 为空返回 nil（调用方回退 meta_stats 静态值）
func (h *H) dbTypes() []dbTypeRow {
	...
	h.DB.Model(&model.DbInstance{}).Select("db_type, count(*) as cnt").Group("db_type").Find(&totals)
...
type instanceWithNodes struct {
	*model.DbInstance
	Nodes []model.DbInstanceNode `json:"nodes"`
}

// ListMetaClusters 元数据域集群列表（db_type / environment 过滤 + 实例/节点计数）
func (h *H) ListMetaClusters(c *gin.Context) {
	...
	h.DB.Model(&model.DbInstance{}).Select("cluster_id, count(*) as cnt").Group("cluster_id").Find(&instCnt)
	var nodeCnt []struct {
		ClusterID int64 `gorm:"column:cluster_id"`
		Cnt       int64 `gorm:"column:cnt"`
	}
	h.DB.Raw(`SELECT i.cluster_id AS cluster_id, count(*) AS cnt
		FROM db_instance_node n JOIN db_instance i ON i.id = n.instance_id
		GROUP BY i.cluster_id`).Scan(&nodeCnt)
	instBy, nodeBy := map[int64]int64{}, map[int64]int64{}
	for _, r := range instCnt {
		instBy[r.ClusterID] = r.Cnt
	}
	for _, r := range nodeCnt {
		nodeBy[r.ClusterID] = r.Cnt
	}
	items := make([]metaClusterItem, 0, len(clusters))
	for _, cl := range clusters {
		items = append(items, metaClusterItem{DbCluster: cl, InstanceCount: instBy[cl.ID], NodeCount: nodeBy[cl.ID]})
	}
	envelope.OK(c, gin.H{"items": items, "total": len(items)})
}

// GetMetaCluster 集群详情三级下钻：集群 → 实例（含 OB 租户 extensions）→ 节点（挂各实例名下）
func (h *H) GetMetaCluster(c *gin.Context) {
	...
	if err := h.DB.Where("cluster_id = ?", id).Order("id asc").Find(&instances).Error; err != nil {
		envelope.Internal(c, err)
		return
	}
	nodesBy := map[int64][]model.DbInstanceNode{}
	if len(instances) > 0 {
		ids := make([]int64, 0, len(instances))
		for _, i := range instances {
			ids = append(ids, i.ID)
			nodesBy[i.ID] = []model.DbInstanceNode{}
		}
		var nodes []model.DbInstanceNode
		if err := h.DB.Where("instance_id IN ?", ids).Order("instance_id asc, ordinal asc").Find(&nodes).Error; err != nil {
			envelope.Internal(c, err)
			return
		}
		for _, n := range nodes {
			nodesBy[n.InstanceID] = append(nodesBy[n.InstanceID], n)
		}
	}
	items := make([]instanceWithNodes, 0, len(instances))
	for _, inst := range instances {
		items = append(items, instanceWithNodes{DbInstance: inst, Nodes: nodesBy[inst.ID]})
	}
	envelope.OK(c, gin.H{"cluster": cl, "instances": items})
}
