...
| 实现状态 | §2-§5（chat / 插件配置 / 任务 / 内部契约）**已实现**（apiserver + agentcluster-mock 联调）；§6 collector / §7 remote / §8 数据面契约中两组件**设计态未实现**（白名单表结构已定稿，见 §8.2） |
...
覆盖：注册与准入 → 下发与加载 → 运行调用 → 变更生效与下线。核心决策：注册表入库（D3）、Python 直连 MCP（D4）、轮次边界生效 + 版本留痕（D5）。
...
sequenceDiagram
    ...
    Note over ADM,PG: ① 注册与准入
    ADM->>GO: POST /api/model-configs（+/test）、/api/mcp-servers、/api/skills
    GO->>PG: 落配置表 + audit_logs
    opt MCP 自动发现（二期，MVP 保留加载器接口）
        ADM->>GO: 触发发现
        GO->>PY: POST /agentcluster/discover {server_id}
        PY->>MCP: initialize + tools/list
        MCP-->>PY: 工具清单（含 inputSchema）
        PY-->>GO: POST /internal/registry/tools/draft（ToolDefinition 草案）
        Note over ADM: 人工定级（risk/限流/描述润色）后转 active
    end
    end

    rect rgb(248,248,248)
    Note over PY,MCP: ② 下发与加载（拉取式）
    loop 启动全量 + 周期校验版本
        PY->>GO: GET /internal/agent/configs（模型/MCP/Skill + config_version）
        PY->>GO: GET /internal/agent/registry（ToolDefinition）
        GO-->>PY: 配置快照（明文 api_key 仅内网 + 凭证头）
        PY->>PY: AgentDefinition(YAML) 绑定 model_profile/工具集/skills → 编译 graph
    end
    end

    rect rgb(248,248,248)
    Note over PY,MCP: ③ 运行调用
    opt Skill 按需注入（意图命中才加载）
        PY->>GO: GET /internal/agent/skills/:id
        GO-->>PY: SKILL.md 正文 → 注入上下文技能区
    end
    PY->>MCP: tools/call（stdio 直连 / http，进程内连接池）
    MCP-->>PY: 结果
    PY->>PY: 适配器归一 + output_schema 校验 → 回注 LLM 循环
    PY->>GO: POST /internal/turns/:id/trace（调用留痕 tool_calls）
    end

    rect rgb(248,248,248)
    Note over ADM,PY: ④ 变更生效（轮次边界）与下线
    ADM->>GO: 修改/启停 → config_version 递增 + audit
    PY->>GO: 轮询发现版本变化 → 重建 graph
...
1. **不采用文件同步**（会形成第二事实源，违背存储收口）与 **Go 主动推送**（Python 是无状态多副本，推送要管理注册与重试）；拉取 + 版本号最简。
2. **密钥流向**：明文 api_key 只在 Go→Python 内网链路出现，前端永远只拿脱敏 mask；api_key 落库须信封加密（缺口 #11）。
3. MCP 自动发现产出的草案**必须人工定级**（risk_level / 限流 / description 润色）后才能 active，不自动进入专家候选工具集。
