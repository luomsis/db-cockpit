...
| 状态 | 设计定稿 |
| 实现状态 | 待落地：`tool_definitions` 表与管理 API、`POST /internal/tools/data` 过渡通道（现 501）、8 个 builtin 工具均未实现；实现进度见 docs/ROADMAP.md |
...
1. **注册表入库**：ToolDefinition 落 Go 侧 PG（`tool_definitions` 表 + 管理 API，含 `status(active/shadow/deprecated) + version + shadow_of`），agentcluster 经 `GET /internal/agent/registry` 拉取编译——「注册表是数据不是代码」，与指标映射、db_type 清单同哲学；
2. **准入流水**：注册（MCP 自动发现生成**草案** / 手工录入）→ **人工定级**（risk_level / 限流 / description 润色）→ `active` 进入专家候选；发现流程经 `POST /agentcluster/discover`（Go→Python 连接 Server 调 `tools/list`）与 `POST /internal/registry/tools/draft`（Python→Go 草案回写）；
3. **连接归属**：MCP 客户端连接池由 **agentcluster（Python）直连持有**（stdio 由 Python 副本拉起、http 直连，进程内池）；Go 只下发配置（`GET /internal/agent/configs`）与收审计；独立 MCP 网关进程为二期演进（网络隔离需求出现时迁移）；
